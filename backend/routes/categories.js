...
async function duplicateCategoryName(name, id = null) {
...
router.post("/", auth, async (req, res) => {
  try {
    ...
    if (await duplicateCategoryName(name)) return res.status(409).json({ success: false, message: "Ya existe una categoría con ese nombre." });
    const category = await Category.create({ name, image, emoji, active: req.body.active !== false });
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    if (!update.name) return res.status(400).json({ success: false, message: "El nombre es obligatorio." });
    if (update.image.length > 1500000) return res.status(400).json({ success: false, message: "La imagen es demasiado grande." });
    if (await duplicateCategoryName(update.name, req.params.id)) return res.status(409).json({ success: false, message: "Ya existe una categoría con ese nombre." });
